...
fn main() {
    let matches = App::new("stats")
        ...
        .arg(Arg::with_name("median")
             .long("median")
             .help("Print median (Note: median takes O(n) space to compute"))
        .arg(Arg::with_name("percentiles")
             .short("p")
             .long("percentiles")
             .takes_value(true)
             .use_delimiter(true)
             .help("Print arbitrary percentiles. These should be floats in [0, 100] (Note: any percentile takes O(n) space to compute"))
        .arg(Arg::with_name("mode")
             .long("mode")
             .help("Print mode (Note: mode takes O(n) space to compute"))
        .arg(Arg::with_name("mode-count")
             .long("mode-count")
             .help("Print the number of times the mode occured (Note: mode takes O(n) space to compute"))
        // Formatting
